...
const ProductInformation = (props) => {
  ...
  const handleAddToCart = async () => {
    ...
    let PrevproductData = availableProducts.filter(
    ...
    let curproductData = availableProducts.filter(
      (pro) => pro._id === product._id
    );
  ...
  return (
    <div className="lg:flex">
      ...
      <div className="lg:w-1/2 p-4">
        <h2 className="lg:text-3xl text-2xl font-semibold lg:mb-2">
          {product.name}
        </h2>
        <ProductStarRating
          rating={
            typeof product?.avgRating === "undefined" ? 5 : product.avgRating
          }
        />
        <div className="mb-4">
          <span className="lg:text-2xl text-lg font-bold text-orange-500">
            ₹{product.salePrice.toFixed(2)}
          </span>
          <span className="lg:text-lg text-md text-gray-500 line-through ml-2">
